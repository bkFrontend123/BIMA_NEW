...
export default function PolicyCard(props) {
    ...
    const {customClass, policyLogo, logoWidth, logoHeight, title, listText1, listText2, listText3, price, priceAmount, children, policyName, policyCover, policyPremium, featuresIncluded, industry } = props;
        ...
        <>
            <div className={`${style.policyCol} ${customClass}`}>
                <div className={`${style.policyInstant}`}>
                    <i><Image src={instantIcon} alt="Instant Policy" width={15} height={15} /></i> Instant Policy
                </div>
                <div className={`${style.policyCardHdr}`}>
                    ...
                    <h4>{title}</h4>
                    <p><Image src={star} /> Star Features</p>
                </div>
                ...
                <div className={`${style.policyFeaBtn}`}>
                    <Link href="javascript:void(0);" onClick={handleShow}>View all features</Link>
                </div>
                <div className={`${style.policyPrice} ${`d-${price}`}`}>
                    <span>Rs. {priceAmount}</span> <small>+ GST</small>
                </div>
                <div className={`${style.policyCardFtr} d-flex align-items-center justify-content-between gap-2`}>
                    {children}
                </div>
            </div>
            <div className={`${style.policyOffcanvasOuter}`}>
                <Offcanvas show={show} onHide={handleClose} placement="end" className={`${style.policyOffcanvasDiv}`}>
                    <Offcanvas.Header className={`${style.policyOffcanvasHeaderDiv}`}>
                        <div className={`${style.policyOffcanvasHeader}`}>
                            <Link href="javascript:void(0);" className={`${style.policyOffcanvasClose}`} onClick={handleClose}>
                    ...
                    <Offcanvas.Body className={`${style.policyOffcanvasBodyDiv}`}>
                        <div className={`${style.policyOffcanvasBody}`}>
                            <Tabs
                                defaultActiveKey="featuresIncluded"
                                id="policyOffcanvasFeatures"
                                className={`${style.policyOffcanvasBodyTabs} policyOffcanvasBodyTabs`}
                            >
                                <Tab eventKey="featuresIncluded" title="Features Included">
                                    <div className={`${style.policyOffcanvasFeaList}`}>
                                        <ol>
                                            {featuresIncluded}
                                        </ol>
                                    </div>
